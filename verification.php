...
if (isset($_SESSION['email']) AND isset($_SESSION['mdp'])) {
	...
	$req = $bdd->prepare("SELECT `personnel`.`personnel_mail`,`personnel`.`personnel_password` FROM `gestion_prescription`.`personnel` WHERE `personnel_mail`=:email AND `personnel_password`=:mdp;");
	$req->execute(array(
